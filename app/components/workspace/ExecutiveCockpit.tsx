  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      ...
      <div className="card-elevation p-6 relative overflow-hidden bg-[var(--card)] border border-[var(--border)]">
        ...
        <h3 className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-2">Tình trạng vận hành</h3>
        <div className={`text-4xl font-black ${getHealthColor(data.healthScore)} mb-1 tabular-nums`}>
          {data.healthScore}%
        </div>
      ...
      <div className="card-elevation p-6 bg-[var(--card)] border border-[var(--border)]">
        <h3 className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-2">Dòng tiền & Công nợ</h3>
        <div className="space-y-3">
          <div>
            <div className="text-[10px] text-[var(--text-muted)] mb-1 font-bold">Tổng nợ quá hạn</div>
            <div className="text-xl font-black text-rose-600 tabular-nums">{formatVnd(data.totalOverdue)}</div>
          </div>
          ...
          <div className="flex justify-between items-center text-[9px] font-black text-[var(--text-muted)] uppercase tracking-wider">
      ...
      {/* Portfolio Risk Card */}
      <div className="card-elevation p-6 bg-[var(--card)] border border-[var(--border)]">
        <h3 className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-2">Rủi ro thi công</h3>
        <div className="text-3xl font-black text-[var(--text-primary)] mb-1 tabular-nums">
        ...
        <div className="mt-4 space-y-2">
          {data.topRisks.length > 0 ? (
            ...
            <div className="p-3 text-center text-[var(--text-muted)] text-[9px] font-black uppercase tracking-widest">Không có rủi ro ghi nhận</div>
          )}
        </div>
      </div>
    </div>
