...
export default function EnterpriseAppShell({ children, activeItem }: { children: React.ReactNode; activeItem?: string }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex overflow-hidden">
      ...
      <main className={`erp-page-main flex-1 flex flex-col h-screen overflow-hidden transition-all duration-300 ease-[var(--motion-easing-standard)] ${sidebarCollapsed ? 'pl-0 md:pl-[var(--erp-sidebar-collapsed)]' : 'pl-0 md:pl-[var(--erp-sidebar-width)]'}`}>
        {children}
      </main>
    </div>
  );
