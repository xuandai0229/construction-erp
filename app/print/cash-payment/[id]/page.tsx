...
export default function PrintCashPaymentPage() {
  const params = useParams();
  const id = params?.id as string;
  const audit = useAuditedPrint({
    printType: "CASH_PAYMENT",
    entityId: id,
    route: `/print/cash-payment/${id}`,
    reason: "In phiếu chi tiền mặt",
  });
  ...
  if (!doc) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white text-rose-500 font-bold">
        Không tìm thấy phiếu chi hoặc có lỗi phát sinh.
      </div>
    );
  }
  ...
  return (
    <PrintLayout>
      ...
      <AccountingDocumentHeader
        ...
        documentTitle="PHIẾU CHI TIỀN MẶT (CASH PAYMENT)"
      ...
      <div className="space-y-6 text-zinc-900 font-sans">
        <div className="border border-black p-4 rounded space-y-3 text-xs">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="font-bold text-zinc-700">Họ và tên người nhận tiền:</span>{" "}
              <span className="text-zinc-900 font-extrabold">{doc.partnerName || "Đối tác nhận tiền vãng lai"}</span>
            </div>
            <div>
          ...
          <div className="border-t border-zinc-200 pt-2">
            <span className="font-bold text-zinc-700">Lý do chi tiền:</span>{" "}
      ...
      {/* Signature Block */}
      <SignatureBlock
        recipientTitle="Người nhận tiền"
