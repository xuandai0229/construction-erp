...
export default function PrintInventoryIssuePage() {
  const params = useParams();
  const id = params?.id as string;
  const audit = useAuditedPrint({
    printType: "INVENTORY_ISSUE",
    entityId: id,
    route: `/print/inventory/issue/${id}`,
    reason: "In phiếu xuất kho vật tư",
  });
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white text-zinc-500">
        Đang tải dữ liệu phiếu xuất kho...
      </div>
    );
  }
  ...
  if (!doc) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white text-rose-500 font-bold">
        Không tìm thấy phiếu xuất kho hoặc có lỗi phát sinh.
      </div>
    );
  }
  ...
  return (
    <PrintLayout>
      ...
      <AccountingDocumentHeader
        ...
        documentTitle="PHIẾU XUẤT KHO VẬT TƯ (INVENTORY ISSUE)"
      ...
      <div className="space-y-6 text-zinc-900 font-sans">
        ...
        <div className="border border-black p-4 rounded text-xs space-y-2">
          <div>
            <span className="font-bold text-zinc-700 font-sans">Xuất cho công trình / dự án:</span>{" "}
            <span className="text-zinc-900 font-bold">{doc.project?.name || "N/A"}</span>
          </div>
          {doc.wbs && (
            <div>
              <span className="font-bold text-zinc-700 font-sans">Hạng mục thi công (WBS):</span>{" "}
              <span className="text-zinc-900 font-bold">{doc.wbs.name}</span>
            </div>
          )}
          <div>
            <span className="font-bold text-zinc-700 font-sans">Lý do xuất kho:</span>{" "}
            <span className="text-zinc-900 font-medium italic">{doc.description || "Xuất vật tư thi công"}</span>
          </div>
        </div>
          ...
          <table className="w-full text-xs text-left font-sans border-collapse">
            <thead className="bg-zinc-100 font-bold border-b border-black">
              <tr>
                ...
                <th className="p-2 border-r border-black w-24 text-center">Kho nguồn</th>
              ...
              {doc.lines?.map((line: any, idx: number) => {
                ...
                return (
                  <tr key={idx}>
                    ...
                    <td className="p-2 border-r border-black text-center font-semibold">{line.sourceWarehouse?.code || "-"}</td>
        ...
        {/* Word Translation */}
        <MoneyTextLine amount={totalAmount} />
      </div>

      {/* Signature block */}
      <SignatureBlock
        recipientTitle="Người nhận hàng"
        chiefAccountant="Kế toán trưởng"
        director="Thủ kho xuất"
      />
