...
export default function CostsPage() {
  ...
  const columns: Column<CostRecord>[] = [
    { header: 'Ngày', accessor: cost => formatDate(cost.date), align: 'center', width: '128px' },
    { header: 'Nhà cung cấp & nội dung', accessor: cost => `${cost.supplier || 'Nhiều nhà CC'} - ${cost.note || ''}`, width: '320px' },
    { header: 'Hạng mục WBS', accessor: cost => wbsList.find(w => w.id === cost.wbsId)?.name || 'N/A', width: '240px' },
    {
      header: 'Loại',
    ...
    { header: 'SL', accessor: cost => cost.quantity || 1, align: 'right', width: '80px' },
    { header: 'Đơn giá', accessor: cost => formatVnd(cost.unitPrice || cost.amount), align: 'right', width: '140px' },
    {
      header: 'Chưa thuế',
      accessor: cost => {
        const vatRate = cost.vatRate !== undefined ? cost.vatRate : 10;
    ...
    { header: 'Bảo hành', accessor: cost => <span className="text-amber-500">-{formatVnd(Math.round(cost.retentionAmount || 0))}</span>, align: 'right', width: '140px' },
    { header: 'Hạch toán', accessor: cost => <span className="font-bold">{formatVnd(cost.amount)}</span>, align: 'right', width: '150px' },
    { header: 'Thực thanh toán', accessor: cost => <span className="font-bold text-emerald-500">{formatVnd(Math.round(cost.amount - (cost.retentionAmount || 0)))}</span>, align: 'right', width: '160px' },
    {
      header: 'Trạng thái',
    ...
    <div className="erp-page">
      ...
      <main className={`erp-page-main ${sidebarCollapsed ? 'with-sidebar-collapsed' : 'with-sidebar-expanded'}`}>
        ...
        <div className="erp-content-container animate-fade-in space-y-6">
          <div className="flex flex-col gap-4 border-b border-[var(--border)] pb-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="text-[20px] font-bold text-[var(--text-primary)]">Quản lý chi phí</h1>
              <p className="mt-1 text-[12px] font-bold uppercase text-[var(--text-tertiary)]">Phân tích dòng tiền chi ra và công nợ phải trả</p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <button onClick={() => exportToCsv('ERP_Costs.csv', filteredCosts)} className="h-[36px] rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--card)] px-4 text-[12px] font-bold text-[var(--text-primary)] hover:bg-[var(--muted)]">
                Xuất CSV
              </button>
              <button onClick={() => { setEditingCost(null); setShowAddModal(true); }} className="h-[36px] rounded-[var(--radius-sm)] bg-blue-600 px-4 text-[12px] font-bold text-white hover:bg-blue-500">
                Thêm chi phí mới
              </button>
            </div>
          </div>
          ...
          <EnterpriseSection title="BỘ LỌC CHI PHÍ">
            <EnterpriseFilterBar>
              ...
              <div className="flex items-end">
                <button onClick={() => { setSearch(''); setTypeFilter('ALL'); setStatusFilter('ALL'); }} className="h-[38px] rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--card)] px-4 text-[12px] font-bold text-[var(--text-primary)] hover:bg-[var(--muted)]">
                  Xóa bộ lọc
                </button>
              </div>
            </EnterpriseFilterBar>
          ...
          <EnterpriseSection title="BẢNG CHI PHÍ" subtitle={`Hiển thị ${filteredCosts.length} / ${costs.length} bản ghi`}>
              ...
              <EnterpriseTable
                ...
                footer={
                  <tr className="h-[40px] text-[12px] font-bold text-[var(--text-primary)]">
                    ...
                    <td className="px-4 text-right font-mono tabular-nums text-amber-500">-{formatVnd(totals.retention)}</td>
      ...
      {selectedCost && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setSelectedCost(null)}>
          <div className="w-full max-w-xl" onClick={(event) => event.stopPropagation()}>
            <EnterpriseCard title="CHI TIẾT CHI PHÍ">
              ...
              <div className="flex gap-3 border-t border-[var(--border)] pt-4">
                <button onClick={() => { setEditingCost(selectedCost); setShowAddModal(true); }} className="h-[36px] flex-1 rounded-[var(--radius-sm)] bg-blue-600 px-4 text-[12px] font-bold text-white hover:bg-blue-500">
                  Chỉnh sửa
                </button>
                <button disabled={isProcessing} onClick={() => handleDelete(selectedCost)} className="h-[36px] flex-1 rounded-[var(--radius-sm)] bg-rose-600 px-4 text-[12px] font-bold text-white hover:bg-rose-500 disabled:opacity-50">
                  Xóa bỏ
                </button>
                <button onClick={() => setSelectedCost(null)} className="h-[36px] flex-1 rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--card)] px-4 text-[12px] font-bold text-[var(--text-primary)] hover:bg-[var(--muted)]">
                  Đóng
                </button>
              </div>
              </div>
